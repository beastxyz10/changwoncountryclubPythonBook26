try{
	...
	if(typeof adn_fp_flag=="undefined") var adn_fp_flag="";
}catch(e){}
...
try{
	...
	if ( (navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1)) {
		...
		c.charset='UTF-8';
		c.defer=true;
		c.src = "https://cdn.jsdelivr.net/npm/promise-polyfill@8/dist/polyfill.min.js";
...
try{
	...
	if(acorss_adn_hstnm!=across_adn_ref_hostname)
	{
		...
		across_adn_inflow_infons.push({"ref_url":across_adn_ref});
...
function fnc_across_adn_fp_info(callbackFunc)
{
	var agent = navigator.userAgent.toLowerCase();
	if ( (navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1)) {
		adn_fp_flag = "ok";
	...
	{
		try{
			var session_across_adn_fp_ids =across_adn_Storage.getItem("across_adn_fp_ids");

			if( session_across_adn_fp_ids=="" || session_across_adn_fp_ids==null ||  typeof  session_across_adn_fp_ids==undefined){
				var c=document.createElement("script");
				c.type="text/javascript";
				c.charset='UTF-8';
				c.defer=true;
				...
				c.onload = function(){
					...
					fpPromise
					  ...
					  .then( function(result) { 
						...
						across_adn_Storage.setItem("across_adn_fp_ids", visitorId);
						adn_fp_flag = "ok";
						callbackFunc();
					} )
				}
			...
			if(adn_health_flag=="ok"){
				if(typeof p !== 'undefined'){
					...
					if(ut.toLowerCase()=="home"){
						try{
							var today=new Date();
							var timestamp_ms = today.getTime();
							var ad_status = 0
							if(across_adn_session_status) ad_status = 1;

							var across_adn_tr_url = "https://log.acrosspf.com/tr/p?ad_ids="+across_rb_adn_uid+"&ui="+ui+"&loc="+encodeURIComponent(across_adn_loc)+"&ref="+encodeURIComponent(across_adn_ref)+"&ad="+ad_status+"&ms="+timestamp_ms+"&rnd="+Math.floor(Math.random()*99999999999);
							try{
								jQuery.ajax({url: across_adn_tr_url,dataType: 'jsonp',success: function(data){}});
							}catch(e){
								try{
									$.ajax({url: across_adn_tr_url,dataType: 'jsonp',success: function(data){}});
								}catch(e){
									try {
										var across_adn3_evt=document.createElement("script");
										across_adn3_evt.type="text/javascript";
										across_adn3_evt.charset='UTF-8';
										across_adn3_evt.defer=true;
										across_adn3_evt.src=across_adn_tr_url;
										document.getElementsByTagName("head")[0].appendChild(across_adn3_evt);
									}catch(e){}
								}
							}
						}catch(e){}
					...
					if(ut.toLowerCase()=="home"){
						...
						try{	
							...
							if(across_adn_items)
							{
								...
								{
									if(adn_prod_view_ix==across_adn_items) item_ix_status = true;
									else{
										localStorage.setItem("adn_prod_view_ix", across_adn_items);
							...
							if(!item_ix_status)
							{
								across_adn_prd_name = u_items["s"];
								if( across_adn_prd_name=="" || across_adn_prd_name==null ||  typeof  across_adn_prd_name==undefined){
								}else{

									var prd_url = "https://kn.acrosspf.com/across_adn_prd_k.ad?ad_ids="+across_rb_adn_uid+"&k="+encodeURIComponent(across_adn_prd_name)+"&t="+Math.floor(Math.random()*99999999999);
									var across_obj_id = fn_across_adn_obj();
									fn_across_adn_frm_send(across_obj_id, prd_url);
									
									var prd_url = "https://log.acrosspf.com/kd?ad_ids="+across_rb_adn_uid+"&ui="+ui+"&k="+encodeURIComponent(across_adn_prd_name)+"&t="+Math.floor(Math.random()*99999999999);
...
function fnc_across_adn_check_infos()
{
	try{
			...
			if( across_adn_inflow=="" || across_adn_inflow==null ||  typeof  across_adn_inflow==undefined){
				...
				if( across_adn_session_info=="" || across_adn_session_info==null ||  typeof  across_adn_session_info==undefined){
					...
					if( session_across_adn_fp_ids=="" || session_across_adn_fp_ids==null ||  typeof  session_across_adn_fp_ids==undefined){
						...
						var session_evt_url = "https://log.acrosspf.com/et?ad_ids="+across_rb_adn_uid+"&fp="+session_across_adn_fp_ids+"&pi="+across_adn_session_info_parse[3]["pi"]+"&ui="+across_adn_session_info_parse[4]["ui"]+"&event=load&ms=0&t="+Math.floor(Math.random()*99999999999);
						...
						try{
							...
							try{
								...
								try {
									var across_adn_evt=document.createElement("script");
									across_adn_evt.type="text/javascript";
									across_adn_evt.charset='UTF-8';
									across_adn_evt.defer=true;
									across_adn_evt.src=session_evt_url;
									document.getElementsByTagName("head")[0].appendChild(across_adn_evt);
								}catch(e){}
							}
						}
					}
...
function across_adn_session_evt(e)
{
	try{
		var session_across_adn_fp_ids =across_adn_Storage.getItem("across_adn_fp_ids");
		var across_adn_session_info=across_adn_Storage.getItem("across_adn_sessions");
		if( across_adn_session_info=="" || across_adn_session_info==null ||  typeof  across_adn_session_info==undefined){
			...
			if( session_across_adn_fp_ids=="" || session_across_adn_fp_ids==null ||  typeof  session_across_adn_fp_ids==undefined){
				...
				var session_evt_url = "https://log.acrosspf.com/et?ad_ids="+across_rb_adn_uid+"&fp="+session_across_adn_fp_ids+"&pi="+across_adn_session_info_parse[3]["pi"]+"&ui="+across_adn_session_info_parse[4]["ui"]+"&event="+e+"&ms="+across_adn_session_ms+"&t="+Math.floor(Math.random()*99999999999);
...
function fn_across_adn_session_init()
{
	across_adn_session_ms = parseInt(across_adn_session_ms)+1;
	across_adn_Storage.setItem("across_adn_sessions_tm", across_adn_session_ms);
}
var across_adn_sessions_tm=across_adn_Storage.getItem("across_adn_sessions_tm");
if( across_adn_sessions_tm=="" || across_adn_sessions_tm==null ||  typeof  across_adn_sessions_tm==undefined){
	across_adn_session_ms=0;

}else{
	across_adn_session_ms = parseInt(across_adn_sessions_tm);
}				

try{
	across_adn_session_init_tm=setInterval(fn_across_adn_session_init, 1);
...
try{
	fnc_adn_health_info(function(){
		...
		fnc_across_adn_fp_info(function(){
			try {
